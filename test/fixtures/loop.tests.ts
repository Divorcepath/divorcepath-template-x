...
describe("loop fixtures", () => {
    it("replaces paragraph loops correctly", async () => {
        ...
        expect(templateText.trim()).toEqual(
            "{#loop_prop}{simple_prop}!{/loop_prop}"
        );
    ...
    it("replaces list loops correctly", async () => {
        ...
        const templateText = await handler.getText(template);
        expect(templateText.trim()).toEqual(
            "{#loop1}Hi{#loop2}{prop}{/loop2}{/loop1}"
        );

        const data = {
            loop1: [
                {
                    loop2: [{ prop: "first" }, { prop: "second" }],
                },
                {
                    loop2: [{ prop: "third" }, { prop: "forth" }],
                },
            ],
        };
    ...
    it("supports boolean conditions", async () => {
        ...
        const templateText = await handler.getText(template);
        expect(templateText.trim()).toEqual(
            "{#loop_prop1}hi!{#condition1}yes!{/}{#condition2}no!{/}{/}"
        );
    ...
    it("ignores the name of the closing tag", async () => {
        ...
        const template = readFixture(
            "loop - nested - ignore closing tag name.docx"
        );
        const templateText = await handler.getText(template);
        expect(templateText.trim()).toEqual(
            "{#loop_prop1}hi!{#loop_prop2}{simple_prop}!{/some_name}{/}"
        );
    ...
    it("replaces nested loops correctly", async () => {
        ...
        const templateText = await handler.getText(template);
        expect(templateText.trim()).toEqual(
            "{#loop_prop1}hi!{#loop_prop2}{simple_prop}!{/loop_prop2}{/loop_prop1}"
        );
    ...
    it("replaces nested conditions correctly", async () => {
        ...
        const templateText = await handler.getText(template);
        expect(templateText.trim()).toEqual(
            "{# condition a}{# condition b}{yes}{/}{# condition c}{no}{/}{/}"
        );
    ...
    it("replaces a loop inside a condition inside a loop correctly", async () => {
        ...
        const templateText = await handler.getText(template);
        expect(templateText.trim()).toEqual(
            "{# teams}{# display}{name}:{# members}First Name: {name}, Last Name: {surname}{/ members};{/}{/ teams}"
        );
        ...
        expect(docText).toEqual(
            "Team A:First Name: Bryson, Last Name: Pike;First Name: Beatriz, Last Name: Schmitt;Team C:First Name: Fionn, Last Name: Lyons;First Name: Pedro, Last Name: Compton;"
        );
    ...
    it(
        "replaces nested loops fast enough",
        async () => {
            const handler = new TemplateHandler();
            ...
            const data = {
                loop_prop1: [
                    {
                        loop_prop2: [{ simple_prop: "some string" }],
                    },
                ],
            };
            ...
            for (let i = 0; i < maxOuterLoop; i++) {
                ...
                for (let j = 0; j < maxInnerLoop; j++) {
                    data.loop_prop1[i].loop_prop2[j] = {
                        simple_prop: (i * maxOuterLoop + j).toString(),
                    };
                }
            }
