  ...
  async getHeaderOrFooter(type) {
    ...
    body.childNodes.forEach(node => {
      if (node.nodeType === XmlNodeType.General && node.nodeName === 'w:p') {
        const pPr = node.childNodes.find(child => child.nodeType === XmlNodeType.General && child.nodeName === 'w:pPr');
        const sectPr = pPr === null || pPr === void 0 ? void 0 : pPr.childNodes.find(child => child.nodeType === XmlNodeType.General && child.nodeName === 'w:sectPr');
        if (sectPr) {
          sectionProps.push(sectPr);
        }
      }
    });

    // Get document level section props
    const docSectPr = last(body.childNodes.filter(node => node.nodeType === XmlNodeType.General && node.nodeName === 'w:sectPr'));
    if (docSectPr) {
      sectionProps.push(docSectPr);
    }

    // Find header/footer reference in section props
    const reference = sectionProps.reduce((found, sectPr) => {
      var _sectPr$childNodes;
      if (found) return found;
      return (_sectPr$childNodes = sectPr.childNodes) === null || _sectPr$childNodes === void 0 ? void 0 : _sectPr$childNodes.find(node => {
    ...
    if (!this._parts[relTarget]) {
      this._parts[relTarget] = new XmlPart("word/" + relTarget, this.zip, this.xmlParser);
    }
...
class DocxParser {
  ...
  joinTextNodesRange(from, to) {
    ...
    if (secondRunNode.parentNode !== paragraphNode) throw new Error(`Can not join text nodes from separate paragraphs. Start position: ${from.textContent}`);
...
const TABLE_LOOP_CONTENT_TYPE = 'table-loop';
class TableLoopPlugin extends TemplatePlugin {
  constructor(...args) {
    super(...args);
    _defineProperty(this, "contentType", TABLE_LOOP_CONTENT_TYPE);
    _defineProperty(this, "loopStrategies", [new LoopTableStrategy()]);
  }
  setUtilities(utilities) {
    this.utilities = utilities;
    this.loopStrategies.forEach(strategy => strategy.setUtilities(utilities));
...
class TemplateCompiler {
  constructor(delimiters, delimiterSearcher, tagParser, plugins, options) {
    this.delimiters = delimiters;
...
class TemplateHandler {
  constructor(options) {
    ...
    this.compiler = new TemplateCompiler(this.options.delimiters, delimiterSearcher, tagParser, this.options.plugins, {
...
exports.Regex = Regex;
exports.SECTIONS_CONTENT_TYPE = SECTIONS_CONTENT_TYPE;
exports.ScopeData = ScopeData;
exports.SectionsPlugin = SectionsPlugin;
exports.TABLE_LOOP_CONTENT_TYPE = TABLE_LOOP_CONTENT_TYPE;
exports.TEXT_CONTENT_TYPE = TEXT_CONTENT_TYPE;
exports.TEXT_NODE_NAME = TEXT_NODE_NAME;
exports.TableLoopPlugin = TableLoopPlugin;
